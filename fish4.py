...
def fetch_seafood_data(query):
    ...
    # 환경 변수에서 API 키를 가져옵니다. 빈 문자열일 경우 실행 환경이 자동으로 키를 채워줍니다.
    api_key = "" 
...
final_query = selected_from_list if selected_from_list else None
if not final_query and st.button("바닷속 정보 찾아보기 🔍"):
...
if final_query:
    with st.spinner(f"🌊 '{final_query}'의 이야기를 바다에서 건져올리고 있어요..."):
        ...
        if data:
            ...
            st.warning("바다가 조금 거친 것 같아요. 잠시 후 다시 검색 버튼을 눌러주세요!")
...
if st.session_state.current_search:
    ...
    <div class="seafood-card">
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
            <h2 style="margin:0; color:#2c3e50;">✨ {res.get('name', final_query)}</h2>
            <span class="badge">📅 제철: {res.get('season', '정보 없음')}</span>
        </div>
        ...
        <p>{res.get('cleaning', '')}</p>
        
        <p class="section-title">🍳 맛있게 즐기는 법</p>
        <ul>
            {"".join([f"<li>{item}</li>" for item in res.get('cooking', [])])}
        </ul>
        <p>💡 <b>찰떡궁합:</b> {res.get('pairing', '')}</p>
        
        <div style="background:#f0f7f4; padding:15px; border-radius:15px; margin-top:15px;">
            <p style="margin:0;"><b>💪 건강 한 스푼:</b> {res.get('nutrition', '')}</p>
        </div>
        
        <p class="section-title">⚠️ 주의하세요</p>
        <p style="font-size:0.9rem; color:#666;">{res.get('warning', '')}</p>
        
        <p class="section-title">📖 바다 이야기</p>
        <p style="font-style: italic; color:#555;">"{res.get('story', '')}"</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Register Button
    current_name = res.get('name', final_query)
    if current_name not in st.session_state.encyclopedia:
        if st.button(f"📖 {current_name} 도감에 등재하기"):
            st.session_state.encyclopedia[current_name] = {
                "date": datetime.now().strftime("%Y-%m-%d %H:%M"),
                "season": res.get('season', '정보 없음'),
                "flavor_preview": res.get('flavor', '')[:50] + "..."
            }
            st.balloons()
            st.rerun()
    else:
